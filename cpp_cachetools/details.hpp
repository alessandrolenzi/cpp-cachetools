...
namespace details {
    ...
    struct _hash<Size, 0, Args...> {
        std::size_t operator()(const std::tuple<Args...> &tuple) {
            return hash_helper(std::get<0>(tuple));
        }
    };
    ...
    struct hash<std::tuple<Args...>> {
        constexpr std::size_t operator()(const std::tuple<Args...> &k) const{
            return _hash<std::tuple_size<std::tuple<Args...>>::value, std::tuple_size<std::tuple<Args...>>::value -1, Args...>()(k);
        }
    };
